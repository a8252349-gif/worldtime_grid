...
export default async function Home({params}:{params:Promise<{lang:string}>}){const {lang}=await params;const locale=lang as Locale;const d=getDictionary(locale);const copy=uiCopy[locale];const sections=(homeData as Record<Locale,Array<{heading:string;paragraphs:string[]}>>)[locale];const featuredGuides=getGuides(locale).slice(0,6);return <>
  ...
  <section className="section"><div className="shell"><h2>{d.guides}</h2><div className="grid-3">{featuredGuides.map(guide=><Link className="card" href={`/${locale}/guides/${guide.slug}/`} key={guide.slug}><h3>{guide.title}</h3><p>{guide.description}</p></Link>)}</div><div className="button-row"><Link className="button-secondary" href={`/${locale}/guides/`}>{d.guides} →</Link></div></div></section>
